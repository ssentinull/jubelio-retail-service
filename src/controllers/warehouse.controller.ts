...
export class WarehouseController {
  ...
  async getWarehouseInventories(request: FastifyRequest, reply: FastifyReply) {
    try {
      const authHeader = request.headers['authorization']
      if (!authHeader) {
        return reply
      ...
      const params = request.params as GetInventoriesRequest
      params.user_id = user.id

      const warehouse = await this.warehouseUsecase.getWarehouseInventories(
        user,
        params,
      )
      ...
      return reply
        .status(200)
        .send(successResponse(warehouse, 'success get warehouse inventories'))
    } catch (error) {
      console.log(error)
      switch (error) {
        case constants.DATA_NOT_FOUND:
